...
from data_converter.utilities.logging import \
    get_conversion_logfile_path
from tqdm import tqdm

from data_converter.configuration.configuration import \
    load_config_setup
from data_converter.utilities.constants import BAR_FORMAT
from data_converter.utilities.get_limited_files import \
    get_limited_files
...
def _process_files(
    ...
    process_timeout: int
        Seconds per attempt before that file 
    ...
    with tqdm(
        ...
        for chunk_index, chunk in enumerate(chunks):
            ...
            while not proc_queue.empty():
                file_path, proc, attempts = proc_queue.get()
                try:
                    returncode = proc.wait(timeout=process_timeout)
                    progress_bar.update()
                    ...
                    attempts += 1
                    if attempts < process_attempts:
                        proc_queue.put((file_path, proc, attempts))
                    else:
                        progress_bar.update()
                        Messenger(logger).debug(
                            f'Converting {file_path.name} failed, skipping...')
    ...
    log_only_messenger.debug(
        f"Method executed in {timer.format_elapsed_time(exec_time)}")
...
def convert_psdata_directory(
    ...
    # num_files = get_and_check(config, int, 'files_limit')
    num_files = config.get('files_limit')
    if num_files is not None:
        num_files = check_type(num_files, int, 'files_limit')
    chunk_size = get_and_check(config, int, 'psdata_tasks', 0)
