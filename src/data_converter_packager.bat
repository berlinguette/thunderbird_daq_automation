...
cd %~dp0
cd ..
rem Edit next line if needed to match your virtual environment
...
python utilities/utilities/packaging/specfile_editor.py data_converter.spec >>%logfile%
echo -- Packaging converter -- >>%logfile%
pyinstaller --clean --noconfirm data_converter.spec >>%logfile% 2>&1
call :Timestamp yyyy,mm,DD,hh,mn,ss,ms,tz
echo - Packaging ends - %yyyy%/%mm%/%DD% %hh%:%mn%:%dd% - >>%logfile%
call deactivate
cd %startfolder%
echo Packaging complete.
echo The packaged converter can be found at %~dp0dist\data_converter.exe
