...
def parquetize_folder(
    ...
    def get_label(file_name: str, batch_number: str = batch_number):
        ...
        return f"pico_{batch_number}s{serial_number.group(1)}"
...
def parquetize_directory(directory: Path, destination: Path, config: Config):
    ...
    - Each of those folders contain one or more Matlab files, one per neutron signal in the buffer
      - Each of these Matlab files must be named in the format 
        "{folder_name}-{signal_number}.mat", where signal_number is padded to 4 digits
    - The generated Parquet files are named in the format "pico_{folder_name}.parquet"
    ...
    # num_folders = get_and_check(config, int, 'files_limit')
    num_folders = config.get('files_limit')
    if num_folders is not None:
        num_folders = check_type(num_folders, int, 'files_limit')
    parquet_tasks = get_and_check(config, int, 'matlab_tasks', 0)
    parquet_files = get_and_check(config, int, 'matlab_files', 0)
